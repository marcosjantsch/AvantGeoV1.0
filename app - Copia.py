...
from core.settings import APP_ICON, APP_TITLE, AUTH_ENABLED, GEO_PATH, LAYOUT, LOGO_PATH, SIDEBAR_STATE
...
def main():
    ...
    if AUTH_ENABLED:
        ...
        if authentication_status is False:
            if not st.session_state.get("auth_login_logged_fail", False):
                log_auth_login(user=username or "unknown", username=username or "", role="unknown", status="FAIL")
        ...
        if not st.session_state.get("auth_login_logged_success", False):
            log_auth_login(user=name or "unknown", username=username or "", role=role or "unknown", status="SUCCESS")
    ...
    sidebar_data = render_sidebar(gdf_full=gdf_full, available_images=st.session_state.get("available_images", []))
    ...
    with tab1:
        ...
        if st.session_state.get("modo_entrada") == "Empresa / Fazenda":
            gdf_filtered = filter_gdf(gdf_full, filtro_shape.get("selected_empresa"), filtro_shape.get("selected_fazenda"))
